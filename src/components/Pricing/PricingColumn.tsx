...
const PricingColumn: React.FC<Props> = ({ tier, highlight }: Props) => {
  ...
  return (
    <div
      className={clsx(
        "w-full max-w-sm mx-auto bg-white rounded-xl border border-gray-200 lg:max-w-full transition duration-300 ease-in-out hover:scale-105",
        { "shadow-lg": highlight },
      )}
    >
      <div className="p-6 border-b border-gray-200 rounded-t-xl">
        ...
        <button
          className={clsx("w-full py-3 px-4 rounded-full transition-colors", {
            "bg-primary text-white hover:bg-red-400 transition duration-300":
              highlight,
            "bg-hero-background hover:bg-gray-200 transition duration-300":
              !highlight,
          })}
        >
